...
from voiceprint.models.style_profile import StyleProfile, profile_store
...
class VoicePrintService:
    ...
    def style_embedder(self):
        if self._style_embedder is None:
            from voiceprint.services.style_embedding import StyleEmbedder
    ...
    def rule_rewriter(self):
        if self._rule_rewriter is None:
            from voiceprint.services.rewrite.rule_rewriter import RuleRewriter
    ...
    def adapt_draft(
        ...
        from voiceprint.services.feedback import build_feedback
    ...
    def _run_rewrite(
        ...
        if use_llm:
            from voiceprint.services.rewrite.llm_rewriter import LlmRewriter, NoApiKeyError
            ...
            if rewriter.is_available():
                try:
                    ...
                    # Log only the exception type. The raw message can echo the
                    # provider's error body, which may contain the user's key.
                    logger.warning(
                        "LLM rewrite failed (%s), using rule rewriter",
                        type(exc).__name__,
                    )
